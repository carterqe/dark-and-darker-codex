...
function validateStringArray(
  value: unknown, maxItems: number, maxLen: number
): string[] | null {
...
export async function POST(request: NextRequest) {
  ...
  if (
    equipment != null &&
    (typeof equipment !== "object" || Array.isArray(equipment) || JSON.stringify(equipment).length > 10000)
  ) {
    return NextResponse.json(
  ...
  if (error) {
    console.error("builds insert error:", error);
    return NextResponse.json(
